...
OLAPStatus TabletMeta::add_stale_rs_meta(const RowsetMetaSharedPtr& rs_meta) {
    // check RowsetMeta is valid
    for (auto rs : _stale_rs_metas) {
        if (rs->version() == rs_meta->version()) {
    ...
    _stale_rs_metas.push_back(rs_meta);
    return OLAP_SUCCESS;
}
...
bool operator==(const TabletMeta& a, const TabletMeta& b) {
    ...
    if (a._stale_rs_metas.size() != b._stale_rs_metas.size()) return false;
    for (int i = 0; i < a._stale_rs_metas.size(); ++i) {
        if (a._stale_rs_metas[i] != b._stale_rs_metas[i]) return false;
    }
